-- Migration: Meeting Composer Support
-- Adds new RPC function to create meetings with fully composed agenda
-- UPDATED: Handle check constraints by using procedural type for placeholders

-- New function that accepts composed agenda items
...
BEGIN
  ...
  IF jsonb_array_length(p_agenda_items) > 0 THEN
    ...
    LOOP
      ...
      -- Update speaker assignment if speaker_id provided
      IF v_item->>'speaker_id' IS NOT NULL THEN
        UPDATE speakers 
        SET meeting_id = v_meeting_id
        WHERE id = (v_item->>'speaker_id')::UUID;
      END IF;
