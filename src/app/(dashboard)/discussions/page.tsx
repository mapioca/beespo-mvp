...
export default async function DiscussionsPage({ searchParams }: DiscussionsPageProps) {
    ...
    let query = supabase
        ...
        .select("id, title, description, category, status, priority, due_date, created_at, updated_at, workspace_id, workspace_discussion_id, created_by", { count: "exact" })
        .eq("workspace_id", profile.workspace_id);

    // Apply search filter
    if (searchQuery) {
        query = query.or(`title.ilike.%${searchQuery}%,description.ilike.%${searchQuery}%,workspace_discussion_id.ilike.%${searchQuery}%`);
    }
    ...
    if (error) {
        ...
        return <div className="p-8">Error loading discussions. Please try again.</div>;
    }
    ...
    allDiscussions?.forEach((d: { status: string; priority: string | null; category: string | null }) => {
        ...
        <>
            ...
            <div className="px-8 pb-8 max-w-7xl mx-auto">
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center font-mono">
                    Page {currentPage} | Showing {discussions?.length || 0} of {count} discussions
