...
interface SpeakersFiltersProps {
    ...
    statusCounts?: Record<SpeakerStatus, number>;
}

const STATUS_OPTIONS: { value: SpeakerStatus; label: string; icon: React.ReactNode }[] = [
    { value: "pending", label: "Pending", icon: <Circle className="h-4 w-4" /> },
    { value: "confirmed", label: "Confirmed", icon: <CheckCheck className="h-4 w-4 text-green-500" /> },
];

export function SpeakersFilters({ onFilterChange, statusCounts }: SpeakersFiltersProps) {
    const [search, setSearch] = useState("");
    const [selectedStatuses, setSelectedStatuses] = useState<SpeakerStatus[]>([]);
    ...
    const toggleStatus = (status: SpeakerStatus) => {
