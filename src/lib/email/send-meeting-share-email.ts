...
export async function sendMeetingShareInviteEmail({
  toEmail,
  inviterName,
  meetingTitle,
  workspaceName,
  permission,
  inviteLink,
}: MeetingShareInviteEmailParams): Promise<{ success: boolean; error?: string }> {
  const permissionLabel = permission === 'editor' ? 'edit' : 'view';

  if (!resend) {
    console.log('------------------------------------------');
    console.log(`📧 [DEV MODE] Meeting Share Invitation Email`);
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      to: toEmail,
      subject: `${inviterName} shared a meeting with you`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
          <head>
            ...
            <div style="max-width: 600px; margin: 0 auto; padding: 48px 24px;">
              ...
              <div style="background-color: #ffffff; border: 1px solid #f3f4f6; border-radius: 16px; padding: 40px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03);">
                ...
                <p style="font-size: 16px; line-height: 24px; color: #4b5563; margin: 0 0 24px 0;">
                  <strong>${inviterName}</strong> from <strong>${workspaceName}</strong> has shared a meeting with you.
                </p>
                ...
                <div style="margin-bottom: 32px;">
                  <a href="${inviteLink}" style="display: inline-block; background-color: #6366f1; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 15px;">
                    View Meeting
                  </a>
                </div>
    ...
    if (error) {
      console.error('Failed to send meeting share invite email:', error);
      return { success: false, error: error.message };
    }

    return { success: true };
  } catch (err) {
    console.error('Failed to send meeting share invite email:', err);
